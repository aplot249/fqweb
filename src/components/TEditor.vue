...
<script>
    ...
    import 'tinymce/plugins/charmap' //特殊字符n
    ...
    export default {
        ...
        data() {
            return {
                init: {
                    ...
                    height: 530,  //注：引入autoresize插件时，此属性失效
                    placeholder: '在这里输入文字',
                    ...
                    content_style: "img {max-width:100%;};code {white-space:normal !important;word-wrap:break-word !important}",
                    // content_css: '@/assets/tiny.css',  //以css文件方式自定义可编辑区域的css样式，css文件需自己创建并引入

                    // images_upload_url: '/article/img/',  //后端处理程序的url
                    ...
                    images_upload_handler: (blobInfo, success, failure) => {
                        ...
                        upload("/image/article/", formData).then(res => success(res.data.image), err => failure('失败', console.log(err)))
                    }
                },
        ...
        watch: {
            ...
            contentValue(newValue) {
                this.$emit('contentModify', newValue)
            }
        },
        methods: {
